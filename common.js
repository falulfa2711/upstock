...
function injectCommonCSS() {
    ...
    style.textContent = `
        ...
        .header-search { position:relative; width:380px; max-width:55vw; flex-shrink:1; }
        .header-search input { width:100%; padding:10px 16px 10px 40px; border-radius:50px; border:2px solid rgba(255,255,255,0.4); background:rgba(255,255,255,0.95); font-family:inherit; font-size:14px; color:#1a0a2e; outline:none; transition:border-color 0.18s, box-shadow 0.18s; direction:rtl; }
        .header-search input::placeholder { color:#888; }
        .header-search input:focus { border-color:#fff; box-shadow:0 0 0 3px rgba(255,255,255,0.3); }
        .search-icon-svg { position:absolute; left:14px; top:50%; transform:translateY(-50%); color:#888; pointer-events:none; z-index:1; }
        .search-dropdown { display:none; position:absolute; top:calc(100% + 8px); right:0; left:0; background:#fff; border:2px solid #e5deff; border-radius:16px; box-shadow:0 16px 48px rgba(124,58,237,0.15); overflow:hidden; z-index:2000; }
        .search-dropdown.open { display:block; }
        .search-item { display:flex; align-items:center; gap:12px; padding:12px 16px; font-size:14px; color:#1a0a2e; text-decoration:none; transition:background 0.15s; cursor:pointer; }
        .search-item:hover { background:#ede9fe; }
        .search-item .si-icon { font-size:20px; }
        .search-item .si-label { font-weight:600; }
        .search-item .si-cat { font-size:12px; color:#6b5b8a; }
