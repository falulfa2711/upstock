...
function injectCommonCSS() {
    ...
    style.textContent = `
        ...
        .slider-inner { display: flex; width: max-content; direction: ltr; animation: sliderMarquee 30s linear infinite; }
        ...
        @keyframes sliderMarquee { to { transform: translateX(-50%); } } /* fallback בלבד */
...
function initProductSlider(categoryName) {
    ...
    inner.style.animation = 'none'; // עוצרים את ה-CSS animation עד שנמדד הרוחב

    // שני RAF מבטיחים שהדפדפן סיים לחשב את הפריסה לפני המדידה
    requestAnimationFrame(() => requestAnimationFrame(() => {
        if (!inner.isConnected) return;
        const allCards = inner.querySelectorAll('.product-card');
        let w = 0;
        for (let i = 0; i < products.length; i++) {
            w += allCards[i].getBoundingClientRect().width
               + parseFloat(getComputedStyle(allCards[i]).marginRight);
        }
        w = Math.round(w);
        // יוצרים @keyframes דינמי עם ערך פיקסלים מדויק (var() לא עובד בתוך @keyframes)
        const kfName = 'sliderKf_' + Date.now();
        const styleEl = document.createElement('style');
        styleEl.textContent = `@keyframes ${kfName} { to { transform: translateX(-${w}px); } }`;
        document.head.appendChild(styleEl);
        inner.style.animation = `${kfName} 30s linear infinite`;
    }));
}
