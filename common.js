...
function injectCommonCSS() {
    ...
    style.textContent = `
        ...
        @keyframes sliderMarquee { 0% { transform: translateX(0); } 100% { transform: translateX(var(--sw, -50%)); } }
...
function initProductSlider(categoryName) {
    ...
    // מתחיל מושהה עד שנמדד הרוחב האמיתי (מניעת קפיצה ראשונית)
    slider.innerHTML = `<div class="slider-inner" style="animation-play-state:paused">${cardsHtml + cardsHtml}</div>`;

    // שני RAF מבטיחים שהדפדפן סיים לחשב את הפריסה
    requestAnimationFrame(() => requestAnimationFrame(() => {
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            w += card.getBoundingClientRect().width + parseFloat(getComputedStyle(card).marginRight);
        }
        inner.style.setProperty('--sw', `-${w}px`);
        inner.style.animationPlayState = ''; // חידוש האנימציה עם הערך המדויק
    }));
}
